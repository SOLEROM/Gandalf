# GANDALF

My custom assistant for LLM agents development and management based on public tools and templates, with a focus on Anthropic's Claude.

![](./logos/logo0.png)

## Tools Map

### help

### plugins

### cmds

### memory

### scripts

### tools

### agents
* [agent team](./21_agentTeam/README.md)

### 02_Plugs
* [ecc](./30_ecc/readme.md) 
* [skillKits] - TBD
* [gstack]- TBD
* [superpowers](./33_superpower/readme.md)
* [cldTenLesRepo](./34_cldTenLesRepo/readme.md)
* [prp-EngContext](./35_conPnP/readme.md)

### 03_my-Sub-Skills

* ```ccpan``` - [create web tui](./11_mySubsSkills/skill_ccpan/README.md)
* ```mdRefactor``` - [rearrange md files](./11_mySubsSkills/skill_mdRefactor/README.md)
* ```gangPatterns``` - [gang patterns](./11_mySubsSkills/skill_gangPatterns/readme.md)
* [targetor](./12_sshs/targetor/README.md) ssh injector





## USAGE:

* deployment : run ```deploy.sh``` 
* skills repo update:
```
git submodule add  https://github.com/SOLEROM/skill_XXXXX 11_mySubsSkills/skill_XXXX
```
* test skill by [dev/skillTester](./dev/skillTests.md)

